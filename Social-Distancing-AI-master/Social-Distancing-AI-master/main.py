...
# Fonksiyon, mouse eventleriyle çağrılır                                                          
...
def calculate_social_distancing(vid_path, net, output_dir, output_vid, ln1):
    ...
    # Kuş bakış ölçeği ayarlama
    # Kuş bakışı sadeceROI gösterecek
    ...
    while True:
        ...
        # yatay ve dikey birim uzunluğu için sonraki 3 noktayı kullanmak (bu durumda 180 cm)
    ...
    ####################################################################################
        ...
        for output in layerOutputs:
            for detection in output:
                ...
                # çerçeve içinde insanları tespit etmek
                if classID == 0:
        ...
        # Burada tüm kutular için sınırlayıcı kutunun alt orta noktasını kullanacağız ve tüm bu alt orta noktaları kuş bakışı görünümüne dönüştüreceğiz.
        person_points = utills.get_transformed_points(boxes1, prespective_transform)
        
        # Burada dönüştürülmüş noktalar (insanlar) arasındaki mesafeyi hesaplayacağız
        ...
        # Risk faktörüne göre insanların etrafına kuş bakışı görünüm ve çerçeve çizimi    
        ...
        # Görüntü ve videoları göster / yaz
        if count != 0:
