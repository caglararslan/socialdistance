...
# Bird Eye View çizme işlevi. Kırmızı, Sarı, Yeşil noktalar insan için riski temsil eder.
# Kırmızı: Yüksek Risk
# Sarı: Düşük Risk
# Yeşil: Güvenli 
...
# iki insan arasındaki risk faktörüne göre kutular belirlenir.
# Kırmızı: Yüksek Risk
# Sarı: Düşük Risk
# Yeşil: Güvenli 
def social_distancing_view(frame, distances_mat, boxes, risk_count):
    ...
    cv2.putText(pad, "Sınırlayıcı kutu, kişiye yönelik risk düzeyini gösterir.", (50, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (100, 100, 0), 2)
    cv2.putText(pad, "-- YÜKSEK RİSK : " + str(risk_count[0]) + " people", (50, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
    cv2.putText(pad, "-- DÜŞÜK RİSK : " + str(risk_count[1]) + " people", (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 1)
    cv2.putText(pad, "-- GÜVENLİ : " + str(risk_count[2]) + " people", (50,  100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
